...
def get_order_items():
    try:
        ...
        return APIResponse.error(
            message=f"Failed to get OrderItem: {e}", status_code=400, error_code=500
        )
...
def get_order_item(order_item_id: int):
    try:
        ...
        if not order_item:
            return APIResponse.error(
                message="Order_item not found", status_code=400, error_code=404
            )
        ...
        return APIResponse.error(
            message=f"Failed to retrieve Order_item: {e}",
            status_code=400,
            error_code=404,
        )
...
def create_order_item():
    try:
        ...
        return APIResponse.success(
            data=new_order_item.serialize(),
            message="Order item created successfully",
            status_code=201,
        )
    except Exception as e:
        return APIResponse.error(
            message=f"Failed to create order item: {e}", status_code=400, error_code=500
        )
